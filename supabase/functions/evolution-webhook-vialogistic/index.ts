      ...
      if (event === 'connect' || (!existing && event !== 'terminate')) {
        ...
        if (existing) {
          ...
          await supabase.from('calls').insert({
            ...
            direction, status, call_type: 'voice',
            started_at: ts, connected_at: status === 'connected' ? ts : null,
...
async function processMetaCallStatuses(statuses: any[]): Promise<void> {
  ...
  for (const s of statuses) {
    ...
    await supabase.from('calls').update(patch).eq('id', existing.id);
    console.log(`📞 [Meta call status VIALOGISTIC] ${raw} → ${patch.status} (${waCallId})`);
  }
}
 ...
 */
const CAPTURE_COMPANY_ID = 'e3ad9c68-cf12-4e39-a12d-3f3068e975a0';
...
async function getOrCreateContact(
  ...
  // ISOLAMENTO: a empresa resolvida precisa ser a VIALOGISTIC. Um vínculo
  // errado em whatsapp_instances nunca pode criar contato na outra empresa.
  ...
  if (!/vialogistic/i.test(resolvedCompanyName) || /viainfra/i.test(resolvedCompanyName)) {
    throw new Error(`VIALOGISTIC company resolution failed for instance: ${instanceName || 'unknown'} (resolved: ${resolvedCompanyName || 'unknown'})`);
  }
...
async function triggerBotResponse(supabase: any, conversationId: string, messageContent: string, remoteJid: string, instanceName: string, inboundExternalId: string | null = null) {
  ...
  // ============================================================
  const EXPECTED_COMPANY_ID = 'e3ad9c68-cf12-4e39-a12d-3f3068e975a0';
  ...
  const companyDisplayName = 'ViaLogistic';
...
async function sendMetaTextMessage(phoneNumber: string, text: string): Promise<boolean> {
  const token = Deno.env.get('META_ACCESS_TOKEN_VIALOGISTIC');
  const phoneNumberId = META_PHONE_NUMBER_ID_VIALOGISTIC;
...
async function sendEvolutionMessageSafe(instanceName: string, phoneNumber: string, text: string) {
  ...
  // 1) API oficial da Meta (canal atual da VIALOGISTIC)
