...
serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    const expected = Deno.env.get('META_VERIFY_TOKEN_VIAINFRA');
...
async function processMetaCallEvent(payload: any): Promise<boolean> {
  try {
    ...
    // VIAINFRA company
    const { data: company } = await supabase.from('companies').select('id').ilike('name', '%viainfra%').maybeSingle();
    if (!company) { console.warn('[Meta calls] Empresa VIAINFRA não encontrada'); return true; }
    ...
    for (const c of calls) {
      ...
      console.log(`📞 [Meta calls] ${event} ${waCallId} (${direction}) processado`);
    }
    return true;
  } catch (e) {
    console.error('[Meta calls] Erro:', e);
    return true;
  }
}
...
async function processMessageUpdate(supabase: any, webhook: EvolutionWebhook) {
  ...
  // Track delivery status for ALL messages (individual + group) if we have a keyId and status
  ...
  // Skip groups and @lid for LID resolution
